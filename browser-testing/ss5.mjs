...
await d.screenshot({ path: '/tmp/v5-desk-1.png' });
await d.evaluate(() => window.scrollBy(0, 500));
await new Promise(r => setTimeout(r, 400));
await d.screenshot({ path: '/tmp/v5-desk-2.png' });
await d.close();
...
await new Promise(r => setTimeout(r, 1500));
await m.screenshot({ path: '/tmp/v5-mob-1.png' });
await m.evaluate(() => window.scrollBy(0, 500));
await new Promise(r => setTimeout(r, 400));
await m.screenshot({ path: '/tmp/v5-mob-2.png' });
await m.close();
console.log('Done');
