...
await d.screenshot({ path: '/tmp/v4-desk-1.png' });
await d.evaluate(() => window.scrollBy(0, 600));
await new Promise(r => setTimeout(r, 400));
await d.screenshot({ path: '/tmp/v4-desk-2.png' });
await d.close();
...
await new Promise(r => setTimeout(r, 1500));
await m.screenshot({ path: '/tmp/v4-mob-1.png' });
await m.evaluate(() => window.scrollBy(0, 600));
await new Promise(r => setTimeout(r, 400));
await m.screenshot({ path: '/tmp/v4-mob-2.png' });
await m.evaluate(() => window.scrollBy(0, 600));
await new Promise(r => setTimeout(r, 400));
await m.screenshot({ path: '/tmp/v4-mob-3.png' });
await m.close();
console.log('Done');
